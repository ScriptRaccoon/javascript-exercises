 ...
 */
const COMMON_WORDS_DE: string[] = JSON.parse(
	readFileSync("./common-words.de.json", "utf8"),
)

/**
 * Typ für die Wordstatistik eines Textes
 */
type WordStats = {
	words_total: number
	words_unique: number
	words_with_frequencies: [string, number][]
}
...
function word_stats(
	txt: string,
	options: { exclude_common_words?: boolean; min_frequency?: number } = {},
): WordStats {
	...
	for (const word of word_list) {
		if (
			...
			(COMMON_WORDS_DE as readonly string[]).includes(word.toLowerCase())
		) {
			continue
		}
